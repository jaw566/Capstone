...
class ImageDialog(QtWidgets.QMainWindow):
    ...
    def loadConfiguration(self):
        #this is where the config file will be read in and radio buttons remnamed
        with open('config.yaml') as file:
            ...
            for module, packages in modules.items():
                ...
                configGroups.append(list()) #makes the array for the grouping
                
                for button in packages.items():
                    radioBttns.append(button[0]) # an array to keep track of the bttns
                    configGroups[iteration].append(button[0]) #add bttns to their groups
                    #create the button
                    ...
                    self.bttn.clicked.connect(partial(self.saveSelectedOptions,button[0],module))
                    rank_in_grp+=1
                iteration+=1
